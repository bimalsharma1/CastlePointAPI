...
namespace Castlepoint.Text
{
    public class ExcelHelper
    {
        ...
        internal static string GetColumnIdFromCellReference(Cell c)
        {
            string columnId = "";
            int number;
            ...
            if (theCell != null)
            {
                ...
                if (ExcelHelper.IsDateTimeCell(bookPart, theCell))
                {
                    try
                    {
                        ...
                        double tryGetValue = 0;
                        bool isDoubleValue = double.TryParse(value, out tryGetValue);
                        if (isDoubleValue)
                        {
                            DateTime newDate = DateTime.FromOADate(tryGetValue);
                            value = newDate.ToString();
                            return value;
                        }
                        else
                        {
                            Console.WriteLine("GetCellValue (IsDateTimeCell) failed Double check for: " + value);
                            return value;
                        }
                    }
                    catch (Exception exDateTimeCell)
                    {
                        Console.WriteLine("GetCellValue (IsDateTimeCell): " + exDateTimeCell.Message);
                        return value;
                    }
                }
